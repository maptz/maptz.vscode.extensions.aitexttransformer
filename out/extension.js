...
exports.deactivate = exports.activate = exports.callApiMethod = void 0;
...
//     }
// }
async function callOpenAIMethod(prompt, apiKey) {
...
async function callApiMethod(prompt, apiKey) {
    let retval = await vscode_1.window.withProgress({
...
function activate(context) {
    let disposable = vscode.commands.registerCommand(`${projectName}.transformSelectedText`, async () => {
        ...
        const aiService = vscode.workspace.getConfiguration().get(`${projectName}.aiService`);
        if (aiService !== "openAI") {
        ...
        const defaultTransform = vscode.workspace.getConfiguration().get(`${projectName}.defaultTransform`);
        const transformCommand = await vscode_1.window.showInputBox({
        ...
        const secretSkipConfirmation = vscode.workspace.getConfiguration().get(`${projectName}.skipConfirmation`);
        if (!secretSkipConfirmation) {
            const confirmationResult = await vscode_1.window.showQuickPick(['Yes', 'No'], {
        ...
        const defaultFullPrompt = vscode.workspace.getConfiguration().get(`${projectName}.defaultFullPrompt`);
        let fullPrompt = (defaultFullPrompt === null ? "" : defaultFullPrompt);
        fullPrompt = fullPrompt.replace("{0}", transformCommand);
        fullPrompt = fullPrompt.replace("{1}", selectedText);
        const apiKey = vscode.workspace.getConfiguration().get(`${projectName}.apiKey`);
        if (!apiKey) {
        ...
        vscode_1.window.showInformationMessage(`${projectNameFriendly}: A total of ${apiResult.tokenCount} tokens were used for this request.`);
        editor.edit(builder => {
